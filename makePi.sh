...
export MakeBAD=false
export MakeASM=false
...
##### Composicao do executavel e imagem de disco
	...
	cp $Source/$SourceFile.bas $Build/$COMname.bas > /dev/null
	cp $Source/$SourceFile.asc $Build/$COMname.asc > /dev/null
	cp $Source/$SourceFile.bin $Build/$COMname.bin > /dev/null
	...
	then
		...
		for i in $BADaux
		do 
			cp $Source/$i.asc $Build/$i.asc > /dev/null
			cp $Source/$i.bas $Build/$i.bas > /dev/null
		done
	fi
	...
	then
		...
		for i in $ASMaux
		do 
			cp $Source/$i.asm $Build/$i.asm > /dev/null
			cp $Source/$i.bin $Build/$i.bin > /dev/null
		done
	fi
	...
	rm $Source/$SourceFile.bin > /dev/null
	rm $Source/$SourceFile.bas > /dev/null
	rm $Source/$SourceFile.asc > /dev/null
	rm $Build/$SourceFile.sta > /dev/null
	...
	then
		...
		for i in $BADaux
		do 
			rm $Source/$i.asc > /dev/null
			rm $Source/$i.bas > /dev/null
		done
	fi
	...
	if [ -f $Build/$DiskName ] 
	then
        rm $Build/$DiskName > /dev/null
    fi
	if $MakeDOS
	then
		$CopyToFloppy $Tools/msxdos.sys $Tools/command.com $Source/*.asm > /dev/null
		echo -e "basic autoexec.bas" > $Build/autoexec.bat
		$Tdos $Build/autoexec.bat > /dev/null
	fi
